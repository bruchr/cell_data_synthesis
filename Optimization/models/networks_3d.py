...
def define_resnet(opt, input):
    ...
    if opt["mode"] == "train":
        ...
        direction = opt["direction_inference"]
    if direction == "AtoB":
        if input == "A":
            input_nc = opt["A_nc"]
            output_nc = opt["B_nc"]
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, opt, input_nc, output_nc):
        ...
        model = [nn.ReplicationPad3d(3), #nn.ReflectionPad3d(3),
                 nn.Conv3d(input_nc, opt["ngf"], kernel_size=7, padding=0, bias=True),
                 norm_layer(opt["ngf"]),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv3d(opt["ngf"] * mult, opt["ngf"] * mult * 2, kernel_size=3, stride=2, padding=1, bias=True),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [nn.ConvTranspose3d(opt["ngf"] * mult, int(opt["ngf"] * mult / 2), kernel_size=3, stride=2, padding=1, output_padding=1, bias=True),
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, norm_layer):
        conv_block = []
        conv_block += [nn.ReplicationPad3d(1), #nn.ReflectionPad3d(1),
                       nn.Conv3d(dim, dim, kernel_size=3, padding=0, bias=True),
                       norm_layer(dim),
                       nn.ReLU(True),
                       nn.ReplicationPad3d(1), #nn.ReflectionPad3d(1),
                       nn.Conv3d(dim, dim, kernel_size=3, padding=0, bias=True),
                       norm_layer(dim)]
...
def define_patchGAN(opt, input):
    ...
    if direction == "AtoB":
        ...
        if input == "B":
            ...
            input_nc = opt["A_nc"]
    net = PatchGANDiscriminator(opt, input_nc)
    return init_net(net, opt)
...
class PatchGANDiscriminator(nn.Module):
    ...
    def __init__(self, opt, input_nc, ndf=64, n_layers=3):
        ...
        norm_layer = get_norm_layer(opt)
        model = [nn.Conv3d(input_nc, opt["ngf"], kernel_size=4, stride=[2,2,2], padding=1, bias=True),
                 nn.LeakyReLU(0.2, True)]

        for i in range(n_layers-1):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv3d(opt["ngf"] * mult, opt["ngf"] * mult * 2, kernel_size=4, stride=[1,2,2], padding=1, bias=True),
                      norm_layer(opt["ngf"] * mult * 2),
                      nn.LeakyReLU(0.2, True)]

        model += [nn.Conv3d(opt["ngf"] * (2**(n_layers-1)), opt["ngf"] * (2**n_layers), kernel_size=4, stride=1, padding=1, bias=True),
                  norm_layer(opt["ngf"] * (2**n_layers)),
                  nn.LeakyReLU(0.2, True),
                  nn.Conv3d(opt["ngf"] * (2**n_layers), 1, kernel_size=4, stride=1, padding=1, bias=True)]  # 1 channel prediction output
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(self, opt, input_nc, ndf=64, norm_layer=nn.BatchNorm3d):
