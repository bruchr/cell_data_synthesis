...
class CycleGANModel():
    ...
    def __init__(self, opt):
        ...
        if opt["mode"] == "train":
            ...
            self.visualizer.define_loss_group("Discriminators", ["D_A", "D_B", "D_A_Acc", "D_B_Acc"])
            self.visualizer.define_loss_group("Generators", ["G", "G_A", "G_B", "Cycle_A", "Cycle_B", "Idt_A", "Idt_B"])
    ...
    def forward(self):
        ...
        self.images["rec_A"] = self.netG_B(self.images["fake_B"])
        self.images["fake_A"] = self.netG_B(self.images["real_B"])
    ...
    def backward_G(self):
        ...
        self.loss["G_A"] = self.criterionGAN(self.netD_A(self.images["fake_B"]), True, self.device)
        ...
        self.loss["Cycle_B"] = self.criterionCycle(self.images["rec_B"], self.images["real_B"]) * lambda_cycle_B
        # combine losses and calculate gradients
        self.loss["G"] = self.loss["G_A"] + self.loss["G_B"] + self.loss["Cycle_A"] + self.loss["Cycle_B"] + self.loss["Idt_A"] + self.loss["Idt_B"]
    ...
    def optimize_parameters(self, epoch):
        ...
        fake_B = self.fake_B_pool.query(self.images["fake_B"])
        ...
        if self.opt['train_d_every_x']==0 or epoch%self.opt['train_d_every_x']==0:
            ...
            self.loss["D_A"], self.loss['D_A_Acc'] = self.backward_D(self.netD_A, self.images["real_B"], fake_B)
    ...
    def save_results(self, epoch_nr=None):
        ...
        folder = Path.cwd().joinpath("Optimization/checkpoints", self.opt["experiment"])
        if epoch_nr is not None:
            folder = Path(folder.parent / (folder.name + '_{}'.format(epoch_nr)))
    ...
    def save_hyperparameters(self, folder, epoch_nr=None):
        ...
        if epoch_nr is not None:
            ...
            opt_save['experiment'] += '_{}'.format(epoch_nr)
        else:
            opt_save = self.opt
    ...
    def load_models(self):
        if self.opt["resume_training"] == 1 and self.opt["mode"] == "train":
            folder = Path.cwd().joinpath("Optimization/checkpoints", self.opt["experiment"][:-13])
        else:
            folder = Path.cwd().joinpath("Optimization/checkpoints", self.opt["experiment"])
        path = folder.joinpath("models.pth")
    ...
    def train(self, dataloader):
        ...
        for epoch in range(self.opt["epoch_count"], self.opt["learning_rate_fix"] + self.opt["learning_rate_decay"]):  # Epoch loop
            ...
            # Mean for all losses
            for key in self.epoch_loss.keys():
                self.epoch_loss[key] = self.epoch_loss[key] / len(dataloader)
        ...
        for i, data in enumerate(dataloader):
            ...
            if 'inf_patch_size' not in self.opt.keys() or self.opt['inf_patch_size'] is None:
                ...
                with torch.no_grad():
                    ...
                    for sl_in, sl_out, sl_res in zip(slices_in, slices_out, slices_res):
            ...
            folder = os.path.join("Optimization", "checkpoints", self.opt["experiment"], "results_"+ dataset_last_folder, self.opt["direction_inference"])
            fakes_np = tensor2np(fakes, imtype="uint16")
            inference_save_images(fakes_np, folder, filenames)