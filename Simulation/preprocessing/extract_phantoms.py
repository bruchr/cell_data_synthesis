            ...
            if not PhantomExtractor.on_border(image.shape, props[nuclei_nr].bbox):
                ...
                if out is not None:
                    ...
                    while True:
                        ...
                        bbox = props[nuclei_nr].bbox

                        img_out = PhantomExtractor.min_max_normalization(image[bbox[0]:bbox[3],
                                                                               bbox[1]:bbox[4],
                                                                               bbox[2]:bbox[5]], 0, 255)

                        tiff.imwrite(file_name, img_out)
                        tiff.imwrite(file_name[0:-4] + "_label.tif",
                                    label_orig[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]])
                        break
