...
class CameraAcquisitionSimulation:
    ...
    def downsample(img: np.ndarray, px_size_img: Tuple[float, float, float],
                   px_size_desired: Tuple[float, float, float], mode: str = "image", verbose: bool = False):
        """
        ...
        Args:
            ...
            verbose: if set, there is command line output

        Returns:
            The resized image
        """
        if verbose:
            print("Scale with mode " + mode + " " + "-" * 20)
            print("Scale: Input: Min/Max value in image: {} / {}. dtype: {}"
        ...
        if verbose:
            print("Scale: After rescale: Min/Max value in image: {} / {}. dtype: {}"
                  .format(np.min(img), np.max(img), img.dtype))
        ...
        if verbose:
            print("Scale: Output: Min/Max value in image: {} / {}. dtype: {}"
                  .format(np.min(img), np.max(img), img.dtype))
    ...
    def add_gauss_noise(img: np.ndarray, mu: float = 0, sigma: float = 0.01, absolute: bool = False) -> np.ndarray:
        ...
        if absolute:
            normal_distribution = np.abs(normal_distribution)
        img = img.astype(np.uint16) + normal_distribution
        img = np.clip(img, 0, 255).astype(np.uint8)
        return img
    ...
    def run(self, image: np.ndarray, label: np.ndarray, mask_num: int, img_num: int, verbose: bool = False,
        ...
        if self.px_size_img != self.px_size_desired:
            start = time.time()
            image = self.downsample(image, self.px_size_img, self.px_size_desired, mode="image", verbose=verbose)
            label = self.downsample(label, self.px_size_img, self.px_size_desired, mode="label", verbose=verbose)
            duration = time.time() - start
            if verbose:
                print("{:.3f}s ({:.3f}min) needed for downsampling of 1 image and label"
        ...
        for n in range(0, self.num_of_acquisition_iterations):
            ...
            image_n = self.min_max_normalization(image, self.baseline, image.max())
        ...
        if self.num_of_acquisition_iterations > 1:
            image = np.mean(np.array(image_iteration), axis=0).astype(np.uint8)
        else:
